...
p3 = Plots.scatter(alpha_p_dc,color=color_dc,legend=nothing, xlabel = "\$hour\$", ylabel = "\$g_{id}\$")
plot_filename = joinpath(plot_path, "generator_status_dc.pdf")
Plots.savefig(p3, plot_filename)

p4 = Plots.scatter(alpha_p_no_dc,color=color_no_dc,legend=nothing, xlabel = "\$hour\$", ylabel = "\$g_{id}\$")
